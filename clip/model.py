...
class ResidualAttentionBlock_Trans(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
                 text_layer=False, i=0, design_details=None):
        super().__init__()
        ...
        if self.text_layer:
            ...
            self.n_ctx_vision = design_details["vision_ctx"]  # hyperparameter
    ...
    def forward(self, inputs):
        ...
        x = inputs[0]
        deep_prompts = inputs[1]
        counter = inputs[2]
        if self.add_prompt:
            if len(deep_prompts) > 0:
                # This means that deep prompts are turned on
                # Here it behaves differently for text and visual side
                # Forward function is same for both

                if not self.text_layer:
                    # First check if the ith layer needs deep prompts or not
                    if not (counter > len(deep_prompts) - 1):
                        # Remove the outputs produced by learnable tokens of previous layer
                        prefix = x[0:x.shape[0] - self.n_ctx_vision, :, :]
                        # Create/configure learnable tokens of this layer
                        visual_context = deep_prompts[counter]  # extract the correct index
                    ...
                    if not (counter > len(deep_prompts) - 1):
                        ...
                        suffix = x[1 + self.n_ctx_text:, :, :]
                        # Create/configure learnable tokens of this layer
                        textual_context = deep_prompts[counter]
        ...
        if not self.text_layer:
            feat_list = inputs[3]
            x_ = x[1:x.shape[0] - self.n_ctx_vision, :, :].permute(1, 0, 2)  # remove learnable prompt and cls tokens
            feat_list.append(x_ / x_.norm(dim=-1, keepdim=True))
            return [x, deep_prompts, counter, feat_list]
        return [x, deep_prompts, counter]  # return again as a list, so that nn.seq can work
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'TransAgent':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_Trans(width, heads, attn_mask, True,
                                                                           text_layer, i,
                                                                           design_details) if prompts_needed > i
                                             else ResidualAttentionBlock_Trans(width, heads, attn_mask, False,
                                                                                text_layer, i, design_details)
                                             for i in range(layers)])
        else:
...
class VisionTransformer_Trans(nn.Module):
    ...
    def forward(self, x: torch.Tensor, visual_ctx, deep_prompts_vision):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        # are trainable parameters here in whole image encoder.
        if self.VPT_shallow:
            visual_ctx = visual_ctx.expand(x.shape[0], -1, -1).half()
            x = torch.cat([x, visual_ctx], dim=1)
        else:
            assert self.prompt_till_layer_visual == 0
        ...
        # outputs = self.transformer([x, deep_prompts_vision, 0, fa_list, gate, num_exp_per_token])  # third argument is counter
        feat_list = []
        outputs = self.transformer([x, deep_prompts_vision, 0, feat_list])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
        ...
        return x, torch.stack(feat_list)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "TransAgent":
                self.visual = VisionTransformer_Trans(
                    input_resolution=image_resolution,
